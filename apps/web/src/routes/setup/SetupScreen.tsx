...
import { Banner, Button, Card, Field, Pill, Spinner, Tabs, inputClass } from '@epistemics/ui';
...
export function SetupScreen() {
  ...
  if (mode === 'shelf') {
    if (packQ.error) return <Banner tone="bad">{packQ.error}</Banner>;
    if (!packQ.data) return packQ.loading ? <Spinner label="Loading pack" /> : <Banner tone="bad">Pack not found on the shelf.</Banner>;
  }
    ...
    <div className="mx-auto max-w-2xl space-y-4" data-testid="setup-screen" data-step={step}>
      <header>
        ...
        <p className="text-sm text-ink/70">
          {step === 'source' ? 'Pick a pack from the Shelf, name a subject, or upload your own material.'
            : step === 'interview' ? 'A few questions set your retention target, exam cap and starting scaffolding.'
            ...
            : 'The Architect is generating concepts, prerequisite graph, lesson scripts and review items.'}
        </p>
      </header>
        ...
        <Card className="space-y-4">
          ...
          {mode === 'material' ? (
            <div className="space-y-3">
              ...
              ) : <p className="text-xs text-ink/60">Nothing added yet. Your files stay on this device; text is chunked and indexed locally.</p>}
            </div>
          ) : null}
          ...
          <div className="flex items-center gap-3">
            ...
            <Link to="/shelf" className="text-sm text-ink/60 hover:underline">Or pick a pack from the Shelf</Link>
          </div>
        </Card>
      ) : null}
      ...
      {step === 'interview' ? (
        <Card className="space-y-4">
          ...
          <Field label={`Weekly time budget: ${interview.weekly} min`}>
          ...
          <div className="flex items-center gap-3">
            ...
            {busy ? <Spinner label={mode === 'shelf' ? 'Creating cards' : progress.message ?? 'Asking the Architect for an outline'} /> : null}
          </div>
        </Card>
      ) : null}
      ...
      {step === 'build' ? (
        <div className="space-y-3">
          ...
          {progress.phase === 'done' && busy ? <Spinner label="Creating cards" /> : null}
          {error ? (
            <div className="flex items-center gap-3">
