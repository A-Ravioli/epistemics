import { Card, ErrorBanner, Meter, PageHeader, Pill, Skeleton, Stat } from '@epistemics/ui';
...
export function ProgressScreen() {
  ...
  if (q.error) return <div className="mx-auto max-w-3xl"><ErrorBanner title="Could not compute progress" message={q.error} onRetry={q.refresh} /></div>;
  if (!q.data) {
    return (
      <div className="mx-auto max-w-3xl space-y-4" data-testid="progress-loading">
        ...
        <Card>
          ...
          <div className="overflow-x-auto">
            <table className="mt-2 w-full min-w-[32rem] text-xs">
              ...
              <tbody>
                {p.cost.byRole.map((r) => (
                  <tr key={r.role} className="border-t border-line"><td className="py-1">{r.role}</td><td>{r.calls}</td><td>{r.inputTokens}</td><td>{r.cacheRead}</td><td>{r.outputTokens}</td><td>{usd(r.costUsd)}</td><td>{Math.round(r.avgLatencyMs)} ms</td></tr>
                ))}
              </tbody>
            </table>
          </div>
