...
export function AccountScreen() {
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-4" data-testid="account-screen">
      <h1 className="text-xl font-semibold">Account and sync</h1>
      <p className="text-sm text-ink/70">Everything stays on this device until you sign in. Signing in syncs your courses, cards, reviews and sessions to your own Supabase project and to any other device signed in with the same account.</p>
      ...
      <Card className="space-y-3">
        <h2 className="text-base font-semibold">Supabase project</h2>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <span className="text-xs text-ink/60" data-testid="sync-configured">{sync.configured ? 'Configured' : 'Not configured: sync is off'}</span>
        </div>
      </Card>
      ...
      <Card className="space-y-3">
        ...
        {signedIn ? (
          ...
          <>
            ...
            {!sync.configured ? <p className="text-xs text-ink/60">Enter your Supabase project above to enable sign-in.</p> : null}
          </>
        )}
      </Card>
      ...
      <Card className="space-y-3">
        ...
        <dl className="grid grid-cols-2 gap-x-4 gap-y-1 text-sm sm:grid-cols-4">
          <dt className="text-ink/60">State</dt><dd data-testid="sync-state">{signedIn ? status.state : 'signed out'}</dd>
          <dt className="text-ink/60">Pending changes</dt><dd data-testid="outbox-count" data-fresh={pendingFresh ? 'true' : 'false'}>{status.pending}</dd>
          <dt className="text-ink/60">Last push</dt><dd>{when(status.lastPushAt)}</dd>
          <dt className="text-ink/60">Last pull</dt><dd>{when(status.lastPullAt)}</dd>
        </dl>
        {status.lastError ? <p className="text-xs text-red-700 dark:text-red-300" data-testid="sync-error">Last sync error: {status.lastError}</p> : null}
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs text-ink/60">Runs on start, when the app becomes visible, after each session, and every minute while signed in.</span>
        </div>
      </Card>

      <Card className="space-y-3">
        <h2 className="text-base font-semibold">Tutor via Supabase edge function</h2>
        <p className="text-sm text-ink/70">Route Claude calls through the <code>anthropic-proxy</code> function of your project: the Anthropic key lives in the function's secrets, the request carries your session token, and a daily budget is enforced server-side.</p>
        <Toggle label="Use the edge function for the tutor" checked={viaEdge} onChange={(v) => void toggleEdge(v)} />
        {viaEdge && !signedIn ? <p className="text-xs text-ink/60">Sign in to activate; the demo tutor is used until then.</p> : null}
        {viaEdge && sync.settings.supabaseUrl ? <p className="text-xs text-ink/60">Endpoint: {sync.settings.supabaseUrl}/functions/v1/anthropic-proxy</p> : null}
      </Card>
    </div>
  );
}
