...
export async function saveCards(db: Db, list: readonly Card[], now: number = Date.now()): Promise<void> {
...
export async function appendReviewLogs(db: Db, logs: readonly ReviewLogEntry[]): Promise<void> {
  await batchAll(db, logs.map((l) => db.insert(reviewLog).values({
